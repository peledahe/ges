            ...
            <div class="mt-10 bg-white shadow overflow-hidden sm:rounded-lg">
                                    ...
                                    @foreach($steps as $index => $step)
                                        <div class="flex flex-col items-center">
                                            <div class="bg-white px-2">
                                                <div class="h-4 w-4 rounded-full border-2 {{ $index <= $currentStatusIndex ? 'bg-indigo-600 border-indigo-600' : 'border-gray-300' }}"></div>
                                            </div>
                                            <div class="mt-2 text-xs text-gray-500 capitalize">{{ str_replace('_', ' ', $step) }}</div>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                ...
                @else
                    <div class="px-4 py-5 sm:p-6 text-center">
                        ...
                        <p class="mt-1 text-sm text-gray-500">No encontramos una orden activa para la placa {{ $plate }}.</p>
                    </div>
                @endif
            </div>
        @endif
